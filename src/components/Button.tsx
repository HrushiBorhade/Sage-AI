...
function Button() {
  ...
  return (
    <div
      ref={scope}
      className="flex flex-col items-center justify-center mt-12 font-heading"
    >
      <Link href="/chat">
        <button
          onMouseEnter={onButtonClick}
          // onClick={onButtonClick}
          className="relative px-6 py-2 pb-3 text-2xl text-white transition-colors border-4 rounded-full pl-7 border-hero_primary hover:bg-hero_secondary hover:border-hero_secondary"
        >
          <span className="sr-only">Sage</span>
          <span className="block h-8 overflow-hidden" aria-hidden>
            {["S", "a", "g", "e", "✨"].map((letter, index) => (
              <span
                data-letter={letter}
